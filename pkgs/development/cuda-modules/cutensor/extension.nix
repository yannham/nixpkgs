...
#   -- one for each version of CUDA.
{
  ...
  in
    lists.map (trivial.flip trivial.pipe [configEvaluator releaseGrabber]) cutensorVersions;
  ...
  # isSupported :: Manifests -> Boolean
  isSupported = {feature, ...}: (attrsets.attrByPath [pname redistArch "providedLibs" libPath] null feature) != null;

  # With our libPath, we can filter out the cutensor releases that don't support our CUDA version or platform.
  # supportedManifests :: List Manifests
  supportedManifests = builtins.filter isSupported listOfManifests;
...
in
  final: _: let
    ...
    buildCutensorPackage = {
      ...
      fixedDrv = drv.overrideAttrs (prev: {
        buildInputs = prev.buildInputs ++ [final.libcublas.lib];
        meta =
          prev.meta
          // {
            description = "cuTENSOR: A High-Performance CUDA Library For Tensor Primitives";
            homepage = "https://developer.nvidia.com/cutensor";
            maintainers = prev.meta.maintainers ++ [lib.maintainers.obsidian-systems-maintenance];
          };
      });
    in
